...
In this Article I will outline the steps to create a real-world application using propriety and opensource [Large Language Models (LLMs)](https://en.wikipedia.org/wiki/Large_language_model).

Recently, LLMs have gained popularity in the mainstream media, especially with the release of [GhatGPT](https://openai.com/chatgpt) and subsequently [Dall-e](https://openai.com/dall-e-3) by OpenAI. What make these models interesting is their near-human text and image generation capabilities.  However, pretrained Models like GPT-3.5 (the model powering chatGPT) come with limitations that need to be addressed before they can be used in building production-level applications.

## Limitations of LLMs and Mitigation Techniques

Recent works have shown that the performance of LMMs to generate human-like responses can be significantly improved with fine-tuning of the foundational model using techniques like [Supervised instruction fine-tuning](https://docs.nvidia.com/nemo-framework/user-guide/latest/modelguide/sft.html#:~:text=Supervised%20Fine%2DTuning%20(SFT)%20is%20the%20process%20of%20fine,done%20after%20model%20pre%2Dtraining), [Reinforcement Learning by Human Feedback](https://huggingface.co/blog/rlhf) and [Domain-adaptive or task-adaptive continued pre-training](https://ceur-ws.org/Vol-2723/short33.pdf). Software application developers intending to use LMMs as a core part of their workflow are always advised to exercise caution when integrating LLMs into business-critical applications. While the afore-mentioned techniques can boost the performance of LMMs significantly, yet they do come at a considerable time, computer resources and cost. This calls for a more pragmatic approach to improving the usability of LMMs in business-critical domains like healthcare, Finance and critical national infrastructure.

LLMs are mostly trained unsupervised on a large corpus of text that is seldom clean. As LLMs are built and trained to conceptualize the meaning and knowledge contained in the source training data, they also imbibe the social and statistical biases inherent to the source dataset. Additionally, because the knowledge learned by the model is limited by the context of the data it was trained with, LMMs are temporally constrained.

For example, when ChatGPT (Powered by OpenAI GPT-3.5) was prompted with the query on the 23rd December 2023:
...
While the LLMs had mate attempt to temporally contextualize its response, the response highlighted the time-bound aspects of its knowledge.

Additionally, LLMs do not have the cognitive ability (like humans) to discern the logical nuances of complex scenarios. For example, consider the prompt below to the chatGPT-3.5:
...
**I know!!!** Someone will say, does that make LMMs useless and risky to use in business and other critical areas? Absolutely **No**.

Let’s take the second example above. We can help the LMMs to make a better judgement by making our prompt more helpful and precise.
...
A((User Prompt))--> B(Retrieval System)--> C(LLM)--> D((System Output))

```

In the [next post](#) I will explain in details the main components of a RAG system and begin to setup our project structure. See you in the next post.